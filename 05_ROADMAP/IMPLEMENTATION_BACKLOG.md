...
6. **Data retention** — choose the shortest practical event-data retention that still supports Lookla's analytics needs; record the setting and whether it resets on new activity. (Note: `MILESTONE_M01.md` and `RELEASE_CHECKLIST.md` currently assume "14 months" as the answer — re-confirm that's still the intended value against this ticket's "shortest practical" framing before finalizing, and update those docs to match whatever is actually chosen.) Do not edit the live Privacy Policy yet — that happens at Stage 2 activation.
